...
typedef struct{
  ...
  uint16_t  CustomData_In_CHdle;                  /**< DATA_IN_Char handle */
  uint16_t  CustomData_Out_CHdle;                  /**< DATA_OUT_Char handle */
  uint16_t  CustomData_Out_T_CHdle;                  /**< DATA_OUT_Time_Char handle */
...
uint16_t SizeData_In_C = 300;
uint16_t SizeData_Out_C = 512;
uint16_t SizeData_Out_T_C = 128;
...
#define COPY_DATA_IN_CHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x43,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_DATA_OUT_CHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x44,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_DATA_OUT_TIME_CHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x45,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          else if (attribute_modified->Attr_Handle == (CustomContext.CustomData_Out_CHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_DATA_OUT_C_NOTIFY_DISABLED_EVT;
                Custom_STM_App_Notification(&Notification);
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_DATA_OUT_C_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
          ...
          }  /* if (attribute_modified->Attr_Handle == (CustomContext.CustomData_Out_CHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))*/

          else if (attribute_modified->Attr_Handle == (CustomContext.CustomData_Out_T_CHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_DATA_OUT_T_C_NOTIFY_DISABLED_EVT;
                Custom_STM_App_Notification(&Notification);
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_DATA_OUT_T_C_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
          ...
          }  /* if (attribute_modified->Attr_Handle == (CustomContext.CustomData_Out_T_CHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))*/
          ...
          else if (attribute_modified->Attr_Handle == (CustomContext.CustomData_In_CHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
            /* USER CODE BEGIN CUSTOM_STM_Service_1_Char_3_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE */
            Notification.Custom_Evt_Opcode = CUSTOM_STM_DATA_IN_C_WRITE_NO_RESP_EVT;
            Notification.DataTransfered.Length   = attribute_modified->Attr_Data_Length;
            Notification.DataTransfered.pPayload = attribute_modified->Attr_Data;
            Custom_STM_App_Notification(&Notification);
            /* USER CODE END CUSTOM_STM_Service_1_Char_3_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE */
          } /* if (attribute_modified->Attr_Handle == (CustomContext.CustomData_In_CHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   *                                2 for DATA_IN_Char +
   *                                2 for DATA_OUT_Char +
   *                                2 for DATA_OUT_Time_Char +
   *                                1 for My_Switch_Char configuration descriptor +
   *                                1 for DATA_OUT_Char configuration descriptor +
   *                                1 for DATA_OUT_Time_Char configuration descriptor +
   *                              = 14
   *
   ...
   *  DATA_IN_Char
   */
  COPY_DATA_IN_CHAR_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomP2PsHdle,
                          UUID_TYPE_128, &uuid,
                          SizeData_In_C,
                          CHAR_PROP_READ | CHAR_PROP_WRITE_WITHOUT_RESP,
                          ...
                          &(CustomContext.CustomData_In_CHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : DATA_IN_C, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : DATA_IN_C \n\r");
  }
   ...
   *  DATA_OUT_Char
   */
  COPY_DATA_OUT_CHAR_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomP2PsHdle,
                          UUID_TYPE_128, &uuid,
                          SizeData_Out_C,
                          CHAR_PROP_NOTIFY,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_VARIABLE,
                          &(CustomContext.CustomData_Out_CHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : DATA_OUT_C, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : DATA_OUT_C \n\r");
  }
   ...
   *  DATA_OUT_Time_Char
   */
  COPY_DATA_OUT_TIME_CHAR_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomP2PsHdle,
                          UUID_TYPE_128, &uuid,
                          SizeData_Out_T_C,
                          CHAR_PROP_NOTIFY,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_VARIABLE,
                          &(CustomContext.CustomData_Out_T_CHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : DATA_OUT_T_C, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : DATA_OUT_T_C \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_DATA_IN_C:
      ret = aci_gatt_update_char_value(CustomContext.CustomP2PsHdle,
                                       CustomContext.CustomData_In_CHdle,
                                       0, /* charValOffset */
                                       SizeData_In_C, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value DATA_IN_C command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value DATA_IN_C command\n\r");
      }
    ...
    case CUSTOM_STM_DATA_OUT_C:
      ret = aci_gatt_update_char_value(CustomContext.CustomP2PsHdle,
                                       CustomContext.CustomData_Out_CHdle,
                                       0, /* charValOffset */
                                       SizeData_Out_C, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value DATA_OUT_C command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value DATA_OUT_C command\n\r");
      }
    ...
    case CUSTOM_STM_DATA_OUT_T_C:
      ret = aci_gatt_update_char_value(CustomContext.CustomP2PsHdle,
                                       CustomContext.CustomData_Out_T_CHdle,
                                       0, /* charValOffset */
                                       SizeData_Out_T_C, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value DATA_OUT_T_C command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value DATA_OUT_T_C command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_DATA_IN_C:
	  ...
	  Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomP2PsHdle, CustomContext.CustomData_In_CHdle, SizeData_In_C, pPayload);
    ...
    case CUSTOM_STM_DATA_OUT_C:
	  ...
	  Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomP2PsHdle, CustomContext.CustomData_Out_CHdle, SizeData_Out_C, pPayload);
    ...
    case CUSTOM_STM_DATA_OUT_T_C:
	  ...
	  Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomP2PsHdle, CustomContext.CustomData_Out_T_CHdle, SizeData_Out_T_C, pPayload);
